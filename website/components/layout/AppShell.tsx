  ...
  ({ className, navbar, footer, sidebar, children, ...props }, ref) => {
    return (
      <div
        ...
        {...props}
      >
        <a href="#main-content" className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-[9999] focus:px-4 focus:py-2 focus:bg-accent focus:text-[color:var(--color-bg-base)] focus:rounded focus:font-bold focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-base focus:ring-accent">
          Skip to main content
        </a>
        
        {navbar && <header className="sticky top-0 z-50 w-full">{navbar}</header>}
